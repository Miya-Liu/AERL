# Qwen3.5 Model for Customized Archon Engine
# Copied from areal/experimental/models/archon/qwen3_5/model/model.py
...
class GatedDeltaNet(nn.Module):
    ...
    - ``FusedRMSNormGated`` (fla) or ``Qwen35RMSNormGated``
    ...
    def __init__(self, args: Qwen35ModelArgs, layer_idx: int) -> None:
        super().__init__()
        ...
        self.norm = (
            Qwen35RMSNormGated(self.head_v_dim, eps=args.norm_eps)
...
class GatedAttention(nn.Module):
    ...
    def __init__(self, args: Qwen35ModelArgs):
        super().__init__()
        ...
        # Q/K norm — Qwen35 RMSNorm with (1+weight) semantics.
        self.q_norm = Qwen35RMSNorm(self.head_dim, eps=args.norm_eps)
        self.k_norm = Qwen35RMSNorm(self.head_dim, eps=args.norm_eps)
...
# ---------------------------------------------------------------------------
# FeedForward, TransformerBlock, Qwen35Model
# ---------------------------------------------------------------------------
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, model_args: Qwen35ModelArgs):
        super().__init__()
        ...
        self.attention_norm = Qwen35RMSNorm(model_args.dim, eps=model_args.norm_eps)
        self.ffn_norm = Qwen35RMSNorm(model_args.dim, eps=model_args.norm_eps)
...
class Qwen35Model(BaseArchonModel):
    """Qwen3.5 hybrid transformer model."""

    def __init__(self, model_args: Qwen35ModelArgs):
        super().__init__()
        ...
        self.norm = Qwen35RMSNorm(model_args.dim, eps=model_args.norm_eps)
