    ...
    def _convert_to_position_rewards(
        ...
        for token_reward in token_rewards:
            ...
            if not top_k_rewards:
                # Fallback: use the actual token as the sole candidate
                position_rewards.append(
                    PositionRewardInfo(
                        position=step,
                        candidates=[token],
                        candidate_token_ids=[actual_token_id],
                        logprobs=[token_reward.get("student_logp", 0.0)],
                        rewards=[0.0],
                        chosen_index=0,
                    )
                )
            ...
            candidates = [str(tkr.get("token", tkr.get("token_id", ""))) for tkr in top_k_rewards]
