...
**Fix:** Added `_apply_reward_discount_called` flag to `__init__` and check in `apply_reward_discount()`.
...
**Fix:** Added `_is_similar_on_last_message()` helper method and warning logic in `__setitem__`.
...
**Fix:** Created `_set_rewards_internal()` method that doesn't acquire lock, and refactored:
- `set_rewards()` now calls `_set_rewards_internal()` within `with self._lock`
- `set_position_rewards()` now calls `_set_rewards_internal()` instead of `set_rewards()`

This avoids potential deadlock from calling a lock-acquiring method while holding the lock.
...
**Recommendation:** Consider extracting common functionality to a shared base class or utility module.
...
2. **Add validation for dynamically set attributes** - Runtime safety for `interaction.xxx = yyy  # type: ignore`

### Low Priority
3. **Standardize error types and messages** - API consistency
4. **Consider extracting common code to base class** - Reduce duplication
5. **Add unit tests for edge cases** - Both implementations need more test coverage
