...
This module contains training components for on-policy distillation with multi-candidate support.
...
**Parameters**:
- `logprobs`: Log probabilities for all candidates `[seq_len, num_candidates]` or `[seq_len]`
...
- `input_data`: Dictionary containing:
  ...
  - `position_rewards`: Position-wise rewards with candidate info (`PositionRewardInfo` list)
...
2. For position-level GRPO (when `position_rewards` is present and logprobs is 2D):
...
2. Gather new logprobs for all positions: `logprobs[positions, :]` → [num_positions, max_candidates]
3. Compute importance weights: `exp(new_logp - old_logp)` (clipped to max 10.0)
4. Compute advantages from rewards (GRPO normalization): `(rewards - mean) / (std + eps)`
5. Compute weighted loss: `-mean(importance_weights * advantages * new_logprobs)` over candidates
6. Normalize by sum of importance weights per position
7. Average over positions weighted by `loss_mask`

**Important**: This function uses `logprobs` directly from the engine (which has gradients) and does NOT call `gather_logprobs_entropy_multi_candidates` again.
...
**Parameters**:
- `logits`: Model logits with shape `[..., vocab_size]` or `[..., vocab_size/tp]` with TP
...
2. Compute entropy: `entropy = -sum(exp(log_probs) * log_probs, dim=-1)`
3. Gather logprobs at label positions:
...
2. All-reduce to get global sum of exponentials
3. Gather logprobs at local label positions
4. All-reduce to combine results from all ranks
5. Mask out-of-range labels with `-inf`
...
1. **Fresh logprobs with gradients**: The engine computes logprobs from current model logits, preserving gradient flow for backpropagation.

2. **Old logprobs for importance weights**: `PositionRewardInfo.logprobs` stores old logprobs from rollout, used only for computing importance sampling weights (`exp(new_logp - old_logp)`).
...
4. **Memory efficiency**: Chunked processing and tensor parallelism support for long sequences.

## Files

- `loss.py` - Loss functions (`grpo_distill_loss_fn`, `_compute_position_level_grpo_loss`)
...
- `engine/fsdp_engine.py` - `MultiCandidateFSDPEngine` for computing multi-candidate logprobs
