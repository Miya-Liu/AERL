...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json-path",
        default="/dfs/share-groups/foundationmodelgroup/LRM/multimodaldata/image/image_qa_code.json",
        help="Path to image_qa_code.json",
    )
    parser.add_argument(
        "--base-image-path",
        default="/dfs/share-groups/foundationmodelgroup/LRM/multimodaldata/image/image_data",
        help="Base directory containing images",
    )
    parser.add_argument(
        "--output", default="./image_qa_results.json", help="Output path for results"
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of tasks to process"
    )
    parser.add_argument("--user-id", default=None, help="User ID for database")
    parser.add_argument(
        "--model-name", default="Qwen3.5_397B_A17B_FP8", help="Model name to use"
    )
    parser.add_argument(
        "--base-url",
        default="http://10.254.10.192:8443/service-large-64-1772072563849/llm/v1",
        help="Base URL for proxy",
    )
    parser.add_argument(
        "--api-key",
        default="RlkgHzgBa2zbPcQrw96rdn68dr7kXk8Mv84Nhs5Trrk6gfq8Cqw5CcDQ787p6d6bPrAWDrw8b97gq8N7jWWxhasnVP76FD76r8tJ2688mdPnSP7N6V7gl1VLKD9LasJq",
