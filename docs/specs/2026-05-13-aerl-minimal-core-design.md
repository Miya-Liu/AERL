...
| `GET /ready` | Same JSON shape as `/health` when `200` (plus optional `upstream_ok: true|false` if useful). Returns `200` if optional readiness checks pass; `503` if upstream probe fails (when enabled). If upstream probe is disabled, behavior matches `/health`. |
...
**Errors:** Responses from the **upstream** LLM API (including HTTP status and body) are passed through **unchanged** when AERL successfully proxies. Only failures **generated by AERL** (validation, internal errors, webhook failures surfaced as AERL errors) use a stable JSON shape `{ "error": { "code", "message", "request_id" } }` (exact nesting TBD in plan; must include `request_id`).
...
| `AERL_JOB_WEBHOOK_URL` | No | If set, job endpoint forwards JSON to this URL (server-side only; not exposed to clients). Implementation plan must include **HTTP client defaults**: bounded timeout (e.g. 30s), TLS verification on by default, optional `AERL_JOB_WEBHOOK_AUTH` or similar for an outbound auth header. |
...
| 2026-05-13 | Spec review | Upstream vs AERL errors, `/ready` shape, webhook client defaults. |
