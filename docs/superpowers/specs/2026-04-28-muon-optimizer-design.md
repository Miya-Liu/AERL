...
- **Distributed**: Use `SingleDeviceMuonWithAuxAdam` — FSDP2 handles gradient synchronization; the original Muon's `dist.all_gather` is designed for DDP and is incorrect under FSDP
...
2. No `p.grad = torch.zeros_like(p)` for missing grads — let the engine handle gradient accumulation
3. No `assert` on param group key sets — be lenient with extra keys from AReaL's config
