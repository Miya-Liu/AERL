...
standard GRPO loss, position-level GRPO distillation loss, or a weighted combination
of both. This merges the distillation functionality from `TreeDistillPPOTrainer`
directly into `CacheAwarePPOTrainer`, controlled by a config field.
...
- `loss_mode != GRPO` → return `MultiCandidateFSDPPPOActor` (enables
  multi-candidate logprob gathering for position-level rewards)
...
2. Store reference for unpatching in `close()`

### `_cache_aware_prepare_batch`

After tree operations and before returning trajectories, inject loss weights
into each trajectory dict:

- `DISTILL` mode: `traj["rl_loss_weight"] = 0.0`
- `BOTH` mode: `traj["rl_loss_weight"] = self.tree_backup_config.rl_loss_weight`
- Both non-GRPO modes: `traj["distill_loss_weight"] = self.tree_backup_config.distill_loss_weight`
...
`customized_areal/on_policy_distill/training/loss.py` handles all three modes
via weight values:
...
When `position_rewards` is `None` (no teacher configured), `grpo_distill_loss_fn`
falls back to pure GRPO loss regardless of mode.
...
No changes to loss functions or actor patching infrastructure — existing code
handles all modes.

## Backward Compatibility

- Default `loss_mode=GRPO` means existing `CacheAwarePPOTrainer` usage is unchanged
- `TreeDistillPPOTrainer` remains as-is for backward compatibility; new
  functionality is available through `CacheAwarePPOTrainer` with
  `loss_mode=DISTILL` or `loss_mode=BOTH`
