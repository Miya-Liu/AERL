...
This causes nodes that were already trained to be retrained — wasting compute and
...
The recover checkpoint's trained episode list is the single source of truth —
...
3. If loaded successfully, call
   `self.tree_store.mark_episodes_trained(episode_ids)` — this marks only
...
- **Missing or corrupt `trained_episodes.json`**: Fall back to
  `reset_trained_flags()`. Safe default — worst case is retraining some nodes.
...
- The tree checkpoint (`mcts_trees/`) continues to save `_trained` flags in
  its `metadata.json` as before — no change to existing tree checkpoint logic.
