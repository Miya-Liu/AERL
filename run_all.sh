...
# Worker - terminal 1
gnome-terminal -- bash -c 'cd /dfs/share-groups/letrain/zhoujie/le-agent-dev/backend && .venv/bin/python -m dramatiq core.agents.worker core.app.workflow.worker core.triggers.worker core.billing.worker --queues agents sub_agents workflows triggers system --processes 8 --threads 8; exec bash'
